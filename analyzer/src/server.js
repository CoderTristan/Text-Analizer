...
function analyzeReadability(text) {
    ...
    if (rawWords.length === 0 || sentences.length === 0) {
        return {score: 0, gradeLevel: 0, error: 'Insufficient text to analyze'}
    }

    const totalSyllables = rawWords.reduce((sum, word) => sum + syllable(word), 0)
    const avgWordsPerSentence = rawWords.length / sentences.length;
    const avgSyllablesPerWord = totalSyllables / rawWords.length;
    ...
    return {
        readabilityScore: parseFloat(fres.toFixed(2)),
        readabilityGradeLevel: parseFloat(fkgl.toFixed(2)), 
